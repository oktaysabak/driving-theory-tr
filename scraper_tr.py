#!/usr/bin/env python3
"""
Driving Theory Questions Scraper for clickclickdrive.de (Turkish Version)
Scrapes all questions and answers from the Turkish driving theory catalog
"""
...
class DrivingTheoryScraperTR:
    def __init__(self, base_url: str = "https://www.clickclickdrive.de", delay: float = 0.5):
        ...
        self.delay = delay
        # Share the same images directory with English/German scrapers
        self.images_dir = Path("images")
        self.images_dir.mkdir(exist_ok=True)
        # Share the same videos directory with English/German scrapers
    ...
    def download_image(self, image_url: str, question_id: str) -> Optional[str]:
        ...
        try:
            # Check if image already exists (might have been downloaded by other scrapers)
    ...
    def download_video(self, video_url: str, question_id: str) -> Optional[str]:
        ...
        try:
            # Check if video already exists (might have been downloaded by other scrapers)
    ...
    def scrape_themes(self) -> List[Dict[str, str]]:
        """Scrape all themes from the Turkish index page"""
        url = f"{self.base_url}/fragenkatalog/tr"
    ...
    def scrape_chapters(self, theme_url: str) -> List[Dict[str, str]]:
        ...
        chapters = []
        # Look for all links that match Turkish chapter pattern
        # Pattern: /fragenkatalog/tr/theme-slug/chapter-slug
        links = soup.find_all('a', href=re.compile(r'/fragenkatalog/tr/[\d\.]+-[^/]+/[\d\.]+-[^/]+$'))
        ...
        for link in links:
            ...
            if chapter_url:
                ...
                if number_span:
                    ...
                    # Remove "Bölüm" prefix if it exists (Turkish for Chapter)
                    chapter_number = chapter_number.replace('Bölüm ', '').replace('Chapter ', '')
                else:
    ...
    def scrape_questions(self, chapter_url: str) -> List[Dict[str, str]]:
        ...
        questions = []
        # Look for all links that match Turkish question pattern
        links = soup.find_all('a', href=re.compile(r'/fragenkatalog/tr/[^/]+/[^/]+/[\d\.]+-\d+(?:-[A-Z])?$'))
    ...
    def scrape_question_details(self, question_url: str, theme: Dict, chapter: Dict, question_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            for info in question_info:
                ...
                if text.startswith('Soru:'):  # Turkish: "Soru"
                    question_number = text.replace('Soru:', '').strip()
                elif 'Puan' in text:  # Turkish: "Puan"
                    points = text
            ...
            question_data = {
                ...
                'question_number': question_number.replace('Soru: ', ''),
                'points': points,
    ...
    def scrape_all(self):
        ...
        print("Starting Turkish scraper...")
    ...
    def export_json(self, filename: str = "driving_theory_questions_tr.json"):
        """Export data to JSON format"""
        with open(filename, 'w', encoding='utf-8') as f:
            json.dump(self.data, f, indent=2, ensure_ascii=False)
        print(f"Exported to {filename}")
    
    def export_csv(self, filename: str = "driving_theory_questions_tr.csv"):
    ...
    def export_markdown(self, filename: str = "driving_theory_questions_tr.md"):
        """Export data to Markdown format"""
        with open(filename, 'w', encoding='utf-8') as f:
            f.write("# Sürücü Kursu Teorik Sorular (Türkçe)\n\n")
...
def main():
    scraper = DrivingTheoryScraperTR()
    
    # Scrape all data
    scraper.scrape_all()
    
    # Export to all formats with _tr suffix
    scraper.export_json()
    scraper.export_csv()
